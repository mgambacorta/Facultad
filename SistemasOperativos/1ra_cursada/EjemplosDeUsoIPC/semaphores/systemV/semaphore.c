   ...
   { 
	   ...
	   else 
	   {
	      ...
	      if ((j = semop(semid, sops, nsops)) == -1)
	      {
	         ...
	         fprintf(stderr, "Child Process Giving up Control of Track: %d/3 times\n", i+1);
       		...
       		if ((j = semop(semid, sops, nsops)) == -1)
                {
                   ...
                   fprintf(stderr, "Parent Process Giving up Control of Track: %d/3 times\n", i+1);
       	      	...
       	      	sleep(1); /* halt process to allow child to catch semaphor change first */
       	 }
